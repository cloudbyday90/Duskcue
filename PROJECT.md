...
| **Offline Downloads** | Android/iOS mobile offline playback with durable server-prepared packages, protected local storage, and reconnect sync |
...
- **Phase 14 task 11 import merge complete** — the async runner now imports matched migration rows into `user_item_data` with non-destructive watch-state merges, records imported row IDs on `migration_import_log`, and logs per-item import errors without aborting the whole run.
...
| Phase 16a | **Complete** (Tasks 0-12 complete: official-source research/design enrichment; Tauri desktop scaffold repaired; Flutter Android/iOS scaffold created; client contracts + typed errors defined; server selection/onboarding implemented; secure auth/session lifecycle foundations implemented; desktop wrapper tray/deep-link/dialog/native-notification layer implemented; Flutter mobile shell/navigation/browsing surface implemented; mobile playback MVP implemented; foreground mobile SSE/realtime updates implemented; mobile push registration + FCM/APNs/UnifiedPush dispatch implemented; mobile quality profile/probe/QoE reporting + quality picker implemented; mobile account/settings hub implemented; packaging/CI/release smoke workflow and focused mobile tests added) | — |
| Phase 16b | **Complete** (Tasks 0-13 complete: official-source TV/console platform research refreshed; TV domain shell, content IDs, authenticated feed, private cache/ETag semantics, shared TV access scope, BOLA-safe direct resolve behavior, deterministic TV surface ranking, admin diagnostics, bounded availability details, TV surface metrics, playback-ready TV resolve responses, playback-entry contract, shared living-room UX contract, adapter contract, and TV surface fixtures/golden renderer added) | — |
| Phase 16c | **Complete** (Tasks 0-15 complete: official Android/iOS offline-download research and [OFFLINE_DOWNLOADS.md](docs/design/OFFLINE_DOWNLOADS.md) created; v1 is Android/iOS only; manifest-backed hybrid package model selected with HLS/fMP4 canonical and single MP4 as a direct-compatible optimization; download schema added for jobs, packages, package files, device state, and explicit download events; downloads domain routes/DTOs/errors wired; `server_config.downloads` policy group plus BOLA/quota/policy checks added; deterministic planning, durable job lifecycle, package worker, manifest, authenticated package serving, Range transfer, job notifications, mobile manager, protected Android/iOS storage, offline playback, reconnect sync, revocation/expiry invalidation, package renewal, cleanup, user settings, admin policy settings, effective per-user/library overrides, inventory diagnostics, observability metrics, focused Rust/mobile model coverage, local Flutter analyzer/test verification, and manual release verification cases are implemented. Physical Android/iOS transfer validation and database-backed end-to-end lifecycle coverage remain release-gate work.) | — |
| Phase 16d | **In progress** (Tasks 0-10 complete: official platform-readiness research and [CLIENT_PLATFORM_READINESS.md](docs/design/CLIENT_PLATFORM_READINESS.md) created; mandatory/advisory gates for Phases 17-23 defined; [client-contracts.v1.json](docs/api/client-contracts.v1.json) promoted to the shared Phase 16d contract source of truth with required domains, per-route response/cache/pagination/validation/error metadata, SSE event inventory, and verifier-enforced route contract drift checks; [client-binding-targets.v1.json](docs/api/client-binding-targets.v1.json) defines fixture-first/generated-later strategies for TypeScript/Tauri, Dart/Flutter, Kotlin Android/Fire TV, Swift tvOS/iOS, Roku, Tizen/webOS, Windows, and Xbox plus shared adapter requirements and verifier coverage; [fixtures/client/v1](docs/api/fixtures/client/v1/manifest.json) adds versioned success, empty, and denial fixtures across all required client domains; [fixtures/playback/v1](docs/api/fixtures/playback/v1/manifest.json) adds state-machine, track-selection, stream-path, media-session, QoE, cross-device resume, and playback-error conformance fixtures; [fixtures/auth/v1](docs/api/fixtures/auth/v1/manifest.json) adds auth-flow, session-lifecycle, secure-storage, switching, and negative-case conformance fixtures; [fixtures/tv/v1](docs/api/fixtures/tv/v1/manifest.json) adds TV surface, deep-link resolve, platform adapter mapping, and access-revalidation conformance fixtures; [fixtures/accessibility/v1](docs/api/fixtures/accessibility/v1/manifest.json) adds accessibility/input baselines, focus-order tests, remote-navigation tests, platform review checklists, and localization/RTL cases; [CLIENT_DESIGN_ASSETS.md](docs/design/CLIENT_DESIGN_ASSETS.md) and [fixtures/design/v1](docs/api/fixtures/design/v1/manifest.json) add shared UI tokens, source app/placeholder assets, artwork loading rules, string ownership, media-state badges, platform mapping, and verifier coverage; [CLIENT_DIAGNOSTICS.md](docs/design/CLIENT_DIAGNOSTICS.md) and [fixtures/diagnostics/v1](docs/api/fixtures/diagnostics/v1/manifest.json) add client log schema, support bundle sections, redaction rules, privacy classifications, server-correlation IDs, platform export checklists, and verifier coverage; [CLIENT_DEVICE_LAB.md](docs/design/CLIENT_DEVICE_LAB.md) and [fixtures/device-lab/v1](docs/api/fixtures/device-lab/v1/manifest.json) add the device/OS/runtime/media compatibility matrix, Docker `:48027` manual smoke scripts, release-required versus best-effort hardware policy, known platform limitations, hardware-gap tracking, and verifier coverage.) | — |
...
**Phase 12 Task 10 complete:** Overlays admin UI + definition CRUD. The seven overlay CRUD/template service functions (`list_overlays`/`get_overlay`/`create_overlay`/`update_overlay`/`delete_overlay`/`list_templates`/`import_template`) — still `todo!()` stubs after Tasks 1–9 — were implemented as the UI's prerequisite. `update_overlay` uses `sqlx::query_builder::QueryBuilder` for PATCH semantics (sqlx 0.9's `SqlSafeStr` guard rejects `format!()` output); system-overlay delete protection returns `AppError::Conflict`. A new recursive `clients/web/src/lib/components/ConditionBuilder.svelte` (Svelte 5 runes, "Match all/any" toggle, depth-capped nested groups, type-adaptive rule mini-forms) serves the `conditions` JSONB editor and is reusable by the Task 11 collection smart-filter UI (shared rule grammar). The full `clients/web/src/routes/settings/overlays/+page.svelte` admin surface replaces the "Coming in Phase 12" stub: definition list grouped by artwork type, type-aware editor (image/text/backdrop field sets, positioning, group/queue/suppress), 16-variable text-template inserter, live preview against a media item, template browser + JSON import, and Apply Now / Re-apply All bulk operations. `e.currentTarget.value` used over `e.target.value` (Svelte 5 types `currentTarget` to the specific element). `cargo check`, clippy, 583 server tests, `npx svelte-check` (0 errors), and `npm run build` all pass. See [BUILD_ORDER.md](BUILD_ORDER.md) Phase 12 Task 10 and [METADATA_OVERLAYS.md](docs/design/METADATA_OVERLAYS.md) Task 10 Implementation Notes for details.

**Phase 12 Task 11 complete:** Collections admin UI + collection CRUD/items/templates. The eleven collection service functions (`list_collections`/`get_collection`/`create_collection`/`update_collection`/`delete_collection`/`list_collection_items`/`add_collection_items`/`reorder_collection_items`/`remove_collection_item`/`list_templates`/`import_template`) — still `todo!()` stubs after Tasks 1–9 — were implemented as the UI's prerequisite, mirroring the Task 10 overlays precedent. `list_collections` and `update_collection` use `sqlx::query_builder::QueryBuilder` for conditional WHERE/PATCH; system-collection delete protection returns `AppError::Conflict`; `collection_type` changes auto-sync the `is_dynamic`/`is_smart` boolean flags so they never desync from the string discriminator. Item positioning uses 1000-spaced integers (matching playlists); `add_collection_items` uses `MAX(position)+1000`; `ON CONFLICT DO NOTHING` prevents duplicates. Template import is an upsert on name. `clients/web/src/lib/api/collections.js` provides the full API client; `clients/web/src/routes/settings/collections/+page.svelte` replaces the "Coming in Phase 12" stub with the full admin surface: collection list grouped by type (Static/Dynamic/Smart) with enable toggle/edit/delete/sync, type-aware editor (dynamic-config builder dropdown grouped Internal/External, limit, schedule, sync_mode, title_format, include/exclude; `ConditionBuilder` for smart filters), display controls, template browser + JSON import, and Sync All / per-collection sync operations. Reuses `ConditionBuilder.svelte` from Task 10 (shared JSONB rule grammar). `cargo check`, clippy (0 warnings), 583 server tests, `npx svelte-check` (0 errors), and `npm run build` all pass. See [BUILD_ORDER.md](BUILD_ORDER.md) Phase 12 Task 11 and [COLLECTIONS.md](docs/design/COLLECTIONS.md) Task 11 Implementation Notes for details.
...
- [x] TV platform surfaces — Platform-neutral Duskcue TV surface feed with native platform adapters; Android TV / Google TV Watch Next first, Fire TV Watch Activity/catalog integration second, Roku deep-link/Search feed integration third, Samsung Tizen Smart Hub Preview fourth, LG webOS launch/resume fifth, Sony BRAVIA Android/Google TV validation, Apple TV / tvOS Top Shelf sixth, VIZIO partner-gated adapter seventh, PlayStation partner-gated adapter eighth, Xbox UWP adapter ninth, VIDAA next research target. Phase 16b now has the server-owned feed, stable platform IDs, BOLA-safe access scope, deterministic ranking, admin diagnostics, bounded availability metadata, TV surface metrics, and playback-ready resolve responses that fetch current resume state before TV playback entry (see [TV_PLATFORM_SURFACES.md](docs/design/TV_PLATFORM_SURFACES.md)).
...
- [x] SWR / `stale-while-revalidate` strategy — Two-layer design documented in [HTTP_CACHING.md](docs/design/HTTP_CACHING.md): (1) HTTP `Cache-Control: stale-while-revalidate` directive on metadata/artwork endpoints (server-side header, zero JS, gracefully degrades to `max-age` on Safari/old TV WebKit per RFC 9111); supported on Chromium-based Smart TVs (Tizen 6.0+/webOS 5.x+, both 2019–2021+) and all desktop browsers except Safari. Layer 1 is implemented in Pre-v1.0 Hardening Task 1 via route-level `SetResponseHeaderLayer` and SHA-256 ETags on single-resource JSON responses. (2) Client-side SWR pattern via `@tanstack/svelte-query` deferred until Phase 11+ — the existing `svelte/store` pattern is sufficient for current UI complexity. `stale-if-error` intentionally NOT used (no CDN in self-hosted deployment; unsupported by browsers).
...
- [x] Internationalization (i18n) strategy — **i18n-ready at v1.0, English source language, community translations post-launch**, documented in [I18N.md](docs/design/I18N.md). Stack: Paraglide JS 2.20.2 (official SvelteKit i18n, compiler-based, type-safe) for web; Fluent via `fluent-templates` 0.14 (Mozilla, used by Firefox + rust-lang.org; `static_loader!` compile-time embedding, explicit per-call locale for async safety) for server-side notification templates; ICU4X for number/date formatting. Translation platform: Weblate (open source, self-hostable, supports Fluent + JSON + Flutter ARB), with setup documented in [WEBLATE.md](docs/operations/WEBLATE.md). **8 target locales: `en` (source), `fr`, `de`, `es`, `it`, `ar`, `zh-Hans`, `zh-Hant` — covering ~4.3B speakers.** Informal register across all locales (matches Netflix/Spotify consumer convention). AI-assisted initial translations + native-speaker review before UI activation. Locale negotiation: URL prefix + cookie + Accept-Language/preferred-language + base locale (English), plus authenticated persistence in `users.metadata.locale`. RTL supported through Paraglide `getTextDirection()`, `<html dir>` replacement, logical CSS properties, direction-aware drawer/toggle transforms, and CSS-generated mirrored back arrows. Error responses stay English (error code is the programmatic identifier). Phase 13b Task 1 shipped Fluent infrastructure + English notification source; Pre-v1.0 Hardening Task 2 shipped Paraglide web integration; Task 5 shipped `clients/web/messages/en.json` plus 7 preview catalogs, 7 preview Fluent notification bundles, and 8-locale `AVAILABLE_LOCALES`; Task 6 completed Arabic RTL layout review for the current web surface; Task 7 shipped `GET/PUT /api/v1/user/preferences`, server-owned `REVIEWED_UI_LOCALES`, and the reviewed-locale language switcher. `REVIEWED_UI_LOCALES` currently contains only `en`.
...
- [x] Desktop/mobile client platform plan — **Tauri 2 shell plus generated Flutter Android/iOS app**, documented in [DESKTOP_MOBILE_CLIENTS.md](docs/design/DESKTOP_MOBILE_CLIENTS.md). Desktop reuses the existing SvelteKit UI behind a strict Tauri capability/plugin surface and adds only native shell features. Mobile is not a WebView wrapper: it uses Flutter UI with native Android Credential Manager/iOS AuthenticationServices passkeys, Android Media3/iOS AVPlayer playback authority, foreground-only SSE, opt-in FCM/APNs/UnifiedPush registration, secure OS-backed token storage, and `duskcue://` MVP deep links with server-side auth/access revalidation.
- [x] Offline downloads v1 scope and package format — **Android/iOS only with a manifest-backed hybrid package model**, documented in [OFFLINE_DOWNLOADS.md](docs/design/OFFLINE_DOWNLOADS.md). HLS/fMP4 directory packages are canonical for mobile playback, selected audio/subtitle tracks, storyboards, resumable transfer, and per-file repair; single MP4 packages are allowed only as a direct-compatible optimization. Web, desktop, TV, console, and casting offline support are deferred. Revocation is immediate for new jobs and online serving, but fully offline packages are disabled/deleted only after app reconnect or local expiry enforcement.
- [x] Desktop/mobile client contract source of truth — **curated Phase 16a manifest now, generated OpenAPI/JSON Schema later**, documented in [CLIENT_CONTRACTS.md](docs/api/CLIENT_CONTRACTS.md). `docs/api/client-contracts.v1.json` inventories the online desktop/mobile route subset and `scripts/verify-client-contracts.mjs` verifies route/helper drift. Flutter DTOs are handwritten against the manifest for Phase 16a; generated bindings and fixtures move to Phase 16d.
- [x] Desktop/mobile server selection — **canonical `http(s)://<server>:48027` origins with explicit Local, Remote VPN, and Exposed modes**, documented in [DESKTOP_MOBILE_CLIENTS.md](docs/design/DESKTOP_MOBILE_CLIENTS.md). Mobile now tests `/health/ready`, saves known servers and the last-used origin in OS-backed secure storage, rejects `48028`, and requires HTTPS for exposed mode. Desktop has Tauri commands for origin normalization, saved-server persistence, and readiness testing; the web API core can target an explicit selected origin for Tauri static builds.
- [x] Desktop/mobile secure auth/session lifecycle — **OS-backed token storage plus stable client device identifiers**, documented in [DESKTOP_MOBILE_CLIENTS.md](docs/design/DESKTOP_MOBILE_CLIENTS.md) and [AUTH.md](docs/design/AUTH.md). Desktop stores bearer tokens through the OS keyring, not app-data JSON. Mobile stores session token, cached user summary, saved servers, and stable device identifier through `flutter_secure_storage`; it supports password, invite, re-auth, device-linking, passkey method-channel flows, logout/logout-all, session listing, session deletion, and auth-expired local credential clearing.
- [x] Desktop wrapper shell — **Tauri static web build plus tray, dialogs, deep links, and native notifications**, documented in [DESKTOP_MOBILE_CLIENTS.md](docs/design/DESKTOP_MOBILE_CLIENTS.md). Desktop now builds the shared SvelteKit UI with adapter-static for Tauri only, keeps the normal web/Docker adapter-node build, exposes tray actions for window/status/notifications/play-pause/quit, registers `duskcue://` deep links, opens native folder selection for library path entry, and mirrors foreground SSE notifications to OS notifications.
- [x] Desktop/mobile packaging smoke — **Tauri package smoke plus Flutter analysis/tests/builds with signing and privacy placeholders**, documented in [CLIENT_PACKAGING.md](docs/ci/CLIENT_PACKAGING.md). `.github/workflows/client-packaging.yml` runs Tauri debug package smoke across Linux/Windows/macOS and Flutter mobile analysis/tests plus Android debug/release builds; iOS metadata/tests run on macOS with simulator build attempted when the generated target exists. Public distribution still requires protected signing, notarization, final branded icons, and store privacy metadata.
- [x] Implementation debt scheduling — **Hybrid: fold critical debt into consuming phases; defer non-blocking to pre-v1.0 hardening**, documented in [IMPLEMENTATION_DEBT.md](docs/design/IMPLEMENTATION_DEBT.md). Phase 10 absorbs 4 items (SSE + EventBus, image pipeline, artwork endpoint, events store — storyboards are first consumer). Phase 13 absorbs 4 items (Fluent setup, multi-channel dispatch, push devices, webhook — notification system is forcing function). Pre-v1.0 hardening quality items are complete: Cache-Control/ETag headers, Paraglide adoption, faceted search API/UI, Prometheus metrics, 7-locale preview translation files, Arabic RTL layout review, and reviewed-locale activation infrastructure. Database partition management executor is immediate (~1 day, before partitions age out). Phase 13 split contingency pre-documented (13a system/backup/maintenance + 13b notifications/i18n/push) if 16-task Phase 13 becomes a bottleneck. Total debt: ~25-40 days across all items. Per Fowler's quadrant, all strategic decisions are Deliberate + Prudent debt — the most desirable category; this schedule prevents drift to Reckless.
...
- [x] PATCH 3-state nullability — **`serde_with::rust::double_option` is the canonical solution; adopted first in overlays (`UpdateOverlayRequest.library_id`), recommended for later `users`/`libraries`/`collections` adoption.** `Option<T>` collapses JSON "absent" and "null" into one `None`, so the standard PATCH type can't express "clear to NULL." `Option<Option<T>>` with `#[serde(default, with = "::serde_with::rust::double_option")]` restores the three states: `None`=absent (unchanged), `Some(None)`=JSON null (clear), `Some(Some(v))`=set. This requires a conditional-SET `QueryBuilder` (not `COALESCE`, which can't clear since `COALESCE(NULL,col)=col`). Applied to overlay `library_id` only — global↔specific is a documented admin workflow (METADATA_OVERLAYS.md Admin UI capability #4) and the field has no validator constraint. `users`/`libraries`/`collections` PATCH endpoints (`COALESCE`-based or plain `Option<Uuid>`) share the limitation but their clear-to-null workflows aren't documented requirements; they can adopt DoubleOption when needed as a deliberate, tested change. The overlays Task 10 implementation establishes the pattern; collections Task 11 explicitly deferred it (see [COLLECTIONS.md](docs/design/COLLECTIONS.md) Task 11 Deferred items).
...
| Offline Downloads | Complete for Phase 16c (mobile package lifecycle, reconnect sync, revocation/cleanup, settings, overrides, diagnostics, metrics, focused tests, and manual release cases complete) | `download_jobs`, `download_packages`, `download_package_files`, `download_device_state`, `download_events`; `server_config.downloads` JSONB | [OFFLINE_DOWNLOADS.md](docs/design/OFFLINE_DOWNLOADS.md), [DATABASE.md](docs/design/DATABASE.md) |
