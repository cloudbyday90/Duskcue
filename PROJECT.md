...
| Phase 12: Kometa-Like System | **In progress** (Task 1: overlays domain scaffolding; Task 2: compositing pipeline — `services/overlays.rs` with `ab_glyph` + `resvg`, `preview_overlay` endpoint working, 60 tests; Task 3: condition evaluation engine — `services/conditions.rs`, JSONB filter rules with 8 operators, nested AND/OR, 64 tests, preview filters by conditions; Task 4: clean art preservation — `services/clean_art.rs`, content-addressed clean backups, Blake3 config hash for change detection, `composite_and_persist()` single-item pipeline, display-layer integration, 14 tests; Task 5: collections domain scaffolding — five-file domain, `COLL_001`–`COLL_008` mapping, collection/item/template DTOs, validation, routes wired; Task 6: collection builder engine — `services/collections.rs`, seven internal builders, five TMDB builders, manual sync persistence, 11 tests; Task 7: scheduled `collection_sync` worker, scheduler registration, idempotent seed migration, per-collection failure recording) | — |
