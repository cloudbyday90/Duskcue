...
| Phase 6: Metadata Providers | **In Progress** (Tasks 1–12) | — |
| Phase 7: Streaming & Playback | **In Progress** (Task 1) | — |
...
**Phase 7 in progress (Task 1):** Playback domain module (`domains/playback/`) with five-file pattern — 22 handler stubs, 20 service stubs, 24 error variants (PLAY_001–013 + domain-specific), full DTO set (Row/Request/Response types for sessions, user item data, bookmarks, playlists), 20 routes wired into router. `AppError::Playback(#[from] PlaybackError)` added to central error enum with `playback_error_to_http()` mapping. See [BUILD_ORDER.md](BUILD_ORDER.md) Phase 7 for details.
