...
| Phase 13a: System Operations Core | **In progress** (Tasks 2-3 complete: generic `server_config` runtime API; scheduled-task list/get/trigger/cancel/history API; notification cleanup executor) | — |
...
**Phase 12 Task 9 complete:** Poster management core implemented. `services/poster_management.rs` handles safe asset-directory discovery, media/season/collection matching, image validation, persistent artwork copy storage, active primary promotion, lock/unlock, and community JSON pack import. New `domains/posters/` API routes support manual asset scans, community import, artwork locking, and active artwork selection, all gated by `Require<CanManageLibraries>`. `workers/asset_directory_scanner.rs` registers `asset_directory_scan` with a daily 03:00 schedule before overlay application; `20260625_080000_seed_asset_directory_scan_task.sql` seeds existing deployments. Primary source changes delete the relevant `artwork_overlay_state` row so stale overlays are not served. No new dependencies or artwork schema changes. See [BUILD_ORDER.md](BUILD_ORDER.md) Phase 12 Task 9 and [POSTER_MANAGEMENT.md](docs/design/POSTER_MANAGEMENT.md) Implementation Notes for details.
