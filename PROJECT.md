...
| Phase 16c | **In progress** (Tasks 0-1 complete: official Android/iOS offline-download research and [OFFLINE_DOWNLOADS.md](docs/design/OFFLINE_DOWNLOADS.md) created; v1 is Android/iOS only; manifest-backed hybrid package model selected with HLS/fMP4 canonical and single MP4 as a direct-compatible optimization; revocation split into immediate server enforcement and reconnect-bound offline-device enforcement; download schema added for jobs, packages, package files, device state, and explicit download events) | — |
