...
| Phase 2: Database Schema | **Complete** | `dd3f201` |
 | Phase 3: Core Server Infrastructure | **In progress** (Tasks 1–11 complete) | — |
...
**Phase 3 in progress:** Tasks 1–11 complete. `config.rs` — clap CLI with `DUSKCUE_` env vars, config-rs layered merge, platform-aware defaults, environment validation. `state.rs` — `AppState` with `PgPool` + `ArcSwap<RuntimeConfig>` + `Arc<RateLimitState>` + `PrometheusHandle` + `Arc<Vec<IpNet>>` (metrics subnet allowlist), full `RuntimeConfig` with 21 sub-config fields, `load_runtime_config()` DB loader, `NetworkConfig` expanded with `allowed_metrics_subnets`, `LoggingConfig` expanded with `level`/`max_file_size_mb`/`max_files`/`format`. `error.rs` — unified `AppError` enum with 11 generic variants, RFC 9457 `ProblemDetail` response struct, `IntoResponse` impl with environment-aware detail. `middleware.rs` — `RateLimitState` with 5 governor keyed rate limiters, `rate_limit_global()` middleware, CORS/security headers/compression/request ID layer builders, `track_http_metrics` middleware (counter + histogram for all HTTP requests), `metrics_subnet_guard` (CIDR-based IP allowlist for `/metrics`). `extractors.rs` — `AuthenticatedUser` extractor (cookie + Bearer token extraction, session validation stub for Phase 4), `PaginationParams` hybrid cursor/offset with full validation, `AdminOnly` capability guard. `router.rs` — full middleware stack, `/health` endpoint with DB connectivity check, `/metrics` endpoint with subnet guard, domain merge points for 15 domains. `logging.rs` — layered tracing subscriber: `EnvFilter` → `ErrorLayer` → pretty console → JSON file with daily rolling appender (`tracing-appender` + `tracing-error`), `init_metrics()` installs global `metrics` recorder with custom histogram buckets via `metrics-exporter-prometheus`. `lockfile.rs` — `Lockfile` struct with PID-file acquire/release at `{data_dir}/.duskcue.lock`, stale lockfile detection via `sysinfo` process liveness check, `Drop` impl for crash cleanup safety net. `main.rs` — 14-step startup sequence: PgPoolOptions with retry (3 attempts/5s), `sqlx::migrate!()` for automatic schema migration, runtime config loading, PG settings validation (warn only), auth setup state check, metrics recorder initialization, startup lockfile acquisition. Graceful shutdown uses `CancellationToken` + `TaskTracker` with 3-phase sequence (Signal → Drain 30s → Cleanup 90s), PG pool close, double-signal protection. Remaining: startup lockfile, PG settings validation expansion. See [BUILD_ORDER.md](BUILD_ORDER.md) for remaining tasks.
...
- **Bootstrap fields only** — `database_url`, `data_dir`, `cache_dir`, `log_level`, `environment`; everything else is in `server_config`
