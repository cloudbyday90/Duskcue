...
| Phase 3: Core Server Infrastructure | **In progress** (Tasks 1–9 complete) | — |
...
**Phase 3 in progress:** Tasks 1–9 complete. `config.rs` — clap CLI with `DUSKCUE_` env vars, config-rs layered merge, platform-aware defaults, environment validation. `state.rs` — `AppState` with `PgPool` + `ArcSwap<RuntimeConfig>` + `Arc<RateLimitState>`, full `RuntimeConfig` with 21 sub-config fields, `load_runtime_config()` DB loader, `LoggingConfig` expanded with `level`/`max_file_size_mb`/`max_files`/`format`. `error.rs` — unified `AppError` enum with 11 generic variants, RFC 9457 `ProblemDetail` response struct, `IntoResponse` impl with environment-aware detail. `middleware.rs` — `RateLimitState` with 5 governor keyed rate limiters, `rate_limit_global()` middleware, CORS/security headers/compression/request ID layer builders. `extractors.rs` — `AuthenticatedUser` extractor (cookie + Bearer token extraction, session validation stub for Phase 4), `PaginationParams` hybrid cursor/offset with full validation, `AdminOnly` capability guard. `router.rs` — full middleware stack, `/health` endpoint with DB connectivity check, domain merge points for 15 domains. `logging.rs` — layered tracing subscriber: `EnvFilter` → `ErrorLayer` → pretty console → JSON file with daily rolling appender (`tracing-appender` + `tracing-error`). `main.rs` — 14-step startup sequence: PgPoolOptions with retry (3 attempts/5s), `sqlx::migrate!()` for automatic schema migration, runtime config loading, PG settings validation (warn only), auth setup state check. Graceful shutdown uses `CancellationToken` + `TaskTracker` with 3-phase sequence (Signal → Drain 30s → Cleanup 90s), PG pool close, double-signal protection. Remaining: Prometheus `/metrics`, startup lockfile. See [BUILD_ORDER.md](BUILD_ORDER.md) for remaining tasks.
