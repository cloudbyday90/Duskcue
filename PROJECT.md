...
| Phase 16c | **In progress** (Tasks 0-12 complete: official Android/iOS offline-download research and [OFFLINE_DOWNLOADS.md](docs/design/OFFLINE_DOWNLOADS.md) created; v1 is Android/iOS only; manifest-backed hybrid package model selected with HLS/fMP4 canonical and single MP4 as a direct-compatible optimization; revocation split into immediate server enforcement and reconnect-bound offline-device enforcement; download schema added for jobs, packages, package files, device state, and explicit download events; downloads domain shell, routes, DTOs, and `DOWNLOAD_001`-`DOWNLOAD_016` error mapping wired; `server_config.downloads` policy group plus BOLA/quota/policy checks added; deterministic planning endpoint and package manifest response implemented; durable job create/status/cancel endpoints and scheduled package worker added for direct copy, HLS/fMP4 remux/transcode, retries, progress, checksums, events, metrics, and cleanup; authenticated package serving and single-range resumable transfer implemented with device/session revalidation and private cache headers; foreground `download_job_status` SSE plus ready/failed notification dispatch added with coalesced progress and opt-in mobile push fan-out; Flutter mobile download manager shell added with scoped inventory/settings, queue/pause/resume/cancel/delete/delete-all/retry flows, Downloads tab, media-detail queue action, and realtime job status merging; Android/iOS protected local download roots added with backup exclusion, iOS file protection, metadata redaction, and logout/delete cleanup; foreground package materialization plus local MP4/HLS offline playback, local resume/watch event queueing, and offline entry points added; idempotent reconnect sync now submits package state and queued playback events, updates `user_item_data`, and marks expired/revoked packages locally) | — |
