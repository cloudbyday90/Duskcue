...
### Phase 12 Task 10 — Admin UI & Definition CRUD (Complete)

Built the overlay management admin UI at `clients/web/src/routes/settings/overlays/+page.svelte` (replacing the "Coming in Phase 12" stub), the API client at `clients/web/src/lib/api/overlays.js`, and the CRUD service bodies that the UI depends on.

**Backend prerequisite — definition CRUD** — Tasks 1–9 left the seven definition-management service functions as `todo!()` stubs (compositing, conditions, clean art, and the application worker were implemented, but `list`/`get`/`create`/`update`/`delete`/`list_templates`/`import_template` were not). Task 10 fills these in because the UI cannot function without working endpoints:

- **`list_overlays`** — paginated `SELECT` with optional `library_id` / `enabled_only` filters; `ORDER BY applies_to, weight DESC, name`. Returns `OverlayListResponse { items, total }`.
- **`get_overlay`** — single-row fetch; `OverlayError::NotFound` (OVERLAY_001) on miss.
- **`create_overlay`** — `INSERT` with `slug` derived from `name` via the existing `generate_slug()`; defaults from the DDL (`font_family: 'Inter'`, `font_size: 63`, `font_color: '#FFFFFF'`, `applies_to: 'poster'`, `is_enabled: true`); `conditions`/`metadata` default to `{}`; `suppresses` defaults to `{}`.
- **`update_overlay`** — partial PATCH via `sqlx::query_builder::QueryBuilder` conditional-SET (only present fields are pushed to the SET clause); `slug` regenerated only when `name` changes. `library_id` uses `Option<Option<Uuid>>` + `serde_with::rust::double_option` for 3-state PATCH (absent / clear-to-null / set) — see Key decisions below.
- **`delete_overlay`** — `is_system` overlays return `AppError::Conflict` (cannot delete built-ins — disabled instead, per design); `is_enabled` is otherwise untouched (delete is a hard row removal).
- **`list_templates` / `import_template`** — templates are community-provided JSON, not a server-curated catalog: `list_templates` returns installed templates discovered from `overlay_definitions.metadata->>'template_name'` (grouped summary), and `import_template` inserts each `TemplateOverlayEntry` as a new `overlay_definitions` row with a fresh slug and `metadata.template_name` linkage.
- A shared `row_to_response()` (private) converts `OverlayDefinitionRow` → `OverlayDefinitionResponse`, reusing the existing `row_to_definition_row()` mapper used by the preview/compositing path. This avoids a second column-list site and keeps the response shape consistent with the compositing pipeline's expectations.

**API client** (`clients/web/src/lib/api/overlays.js`) — thin wrappers over the `core.js` `request()` helpers matching the established `users.js` / `libraries.js` convention: `listOverlays`, `getOverlay`, `createOverlay`, `updateOverlay`, `deleteOverlay`, `applyOverlays`, `previewOverlay`, `listTemplates`, `importTemplate`. All use the `/api/v1/overlays` base path.

**Admin UI** (`clients/web/src/routes/settings/overlays/+page.svelte`) — a single-page admin surface with four interactive areas, built with Svelte 5 runes and the existing design tokens (`--color-*` vars from `app.css`):

1. **Definition list** — every overlay definition grouped by `applies_to` (poster / backdrop / season_poster / episode_thumb); each row shows name, type badge (image/text/backdrop), group/queue, weight, enabled toggle, and edit/delete actions. System overlays show a `System` badge; the delete button is disabled for them. A filter dropdown narrows by library or enabled state. `CanManageLibraries` gates all write actions (consistent with the server-side `Require<CanManageLibraries>`).

2. **Overlay editor** — a create/edit form that adapts its fields to the selected `overlay_type`: image overlays expose `image_path` + `scale_width`/`scale_height`; text overlays expose `text_template` (with an inline `<<variable>>` inserter listing the 16 documented template variables), `font_family`/`font_size`/`font_color`/`stroke_color`/`stroke_width`; backdrop overlays expose `back_color`/`back_width`/`back_height`/`back_radius`/`back_padding`. All types share the positioning controls (`horizontal_align`/`horizontal_offset`, `vertical_align`/`vertical_offset`) and the `applies_to`/`library_id`/`group_name`/`weight`/`queue_name`/`suppresses` fields. Hex colors use native `<input type="color">` with a text fallback for alpha values (`#RRGGBBAA`). The **Preview** button calls `POST /api/v1/overlays/preview` against a chosen media item and renders the returned WebP beside the form, so the operator sees the composited result before saving.

3. **Condition builder** — a recursive, dropdown-driven editor for the `conditions` JSONB. Renders `{"operator", "rules"}` nodes as grouped cards using **"Match all"/"Match any"** labels (research-confirmed clearer than raw AND/OR) with indentation + shaded backgrounds for nesting, capped at 3 levels. Each leaf rule is a smart mini-form: a field dropdown (the 16 condition-testable fields), an operator dropdown (`eq`/`neq`/`in`/`gt`/`gte`/`lt`/`lte`/`exists`/`matches`) whose options narrow by field type, and a value input that adapts — free text for `eq`/`neq`/`matches`, comma-list for `in`, a number stepper for `gt`/`gte`/`lt`/`lte`, and a boolean toggle for `exists`. Add/remove rule and add nested-group buttons follow the standard Rule Builder pattern. Empty conditions default to `{}` (applies to all items). The raw JSON is shown read-only for transparency.

4. **Template browser & bulk operations** — the **Import template** panel accepts a community-pack JSON (matching `OverlayTemplateImport`), validates client-side before posting to `POST /api/v1/overlays/templates`, and reports the imported count. The **Apply Overlays Now** button calls `POST /api/v1/overlays/apply` (optionally scoped to a library / with `reapply_all`) and surfaces the candidate-target count from the response.

**Key decisions (Task 10):**

- **Single-page surface over multi-route** — overlays are an admin-only tool with tightly coupled editor/preview/condition state; a single Svelte route with internal view state (`list` ↔ `editor`) avoids URL plumbing for transient edit sessions and matches the `settings/subtitles` precedent (one large settings page). The list is the default view; editing opens the editor in place.
- **"Match all/any" over raw AND/OR** — per June 2026 UX research (UX StackExchange nested-predicate builder thread; ui-patterns Rule Builder). Boolean terminology "causes users to ask more questions than it answers"; "all of"/"any of" + shading is the validated clearer presentation.
- **Variable inserter over raw typing** — the 16 `<<variable>>` tokens are inserted via a dropdown button rather than expecting operators to memorize them. Reduces OVERLAY_003 (invalid text template) surface area.
- **Native color input + alpha text fallback** — `<input type="color">` covers the common `#RRGGBB` case; the text field alongside it accepts `#RRGGBBAA` (backdrops use translucent fills like `#00000099`). Avoids pulling a color-picker dependency.
- **`update_overlay` uses `sqlx::query_builder::QueryBuilder` (not `COALESCE`)** — the conditional-SET structure is required for 3-state PATCH (see next decision). `users`/`libraries` use the `COALESCE($n, column)` convention, but `COALESCE(NULL, col) = col` makes it impossible to *clear* a nullable field — an inadequate fit for overlays where `library_id` NULL = "global" and the global↔specific toggle is a documented admin workflow (Admin UI capability #4). The QueryBuilder only pushes a SET clause for present fields, so absent fields are untouched.
- **3-state PATCH via `serde_with::rust::double_option`** — `Option<T>` collapses JSON "absent" and "null" into one `None`, so the standard PATCH type cannot express "clear to NULL." `UpdateOverlayRequest.library_id` is therefore `Option<Option<Uuid>>` annotated `#[serde(default, with = "::serde_with::rust::double_option")]`: `None` = absent (unchanged), `Some(None)` = JSON `null` (clear to global), `Some(Some(uuid))` = set. The QueryBuilder binds the inner `Option<Uuid>` directly (`None`→SQL NULL, `Some`→value). The UI sends a literal `library_id: null` (not an omitted key) when the admin clears the field, so DoubleOption sees `Some(None)`. Applied to `library_id` only (the one field with a documented clear-workflow and no validator constraint); other nullable fields keep plain `Option<T>` since their NULL semantics aren't toggled admin workflows. This is the canonical serde answer to 3-state PATCH (dtolnay's `deserialize_some`, packaged by `serde_with`); the pattern is recommended for later adoption in `users`/`libraries` where their PATCH endpoints share the same limitation.
- **System-overlay delete → Conflict** — there is no dedicated OVERLAY code for this business rule (Task 1 decision); the service returns `AppError::Conflict`, which the UI surfaces by disabling the delete button and showing the `System` badge. Disabling (via the enabled toggle) is the supported customization path.
- **Templates are imported, not browsed remotely** — the design doc envisioned a remote community template catalog, but Duskcue is local-first with no bundling of a remote registry. `GET /api/v1/overlays/templates` therefore lists templates *already installed* (grouped by `metadata.template_name`), and import is via JSON paste/file. A future remote catalog can extend this without API changes.
- **Preview requires a media item** — the compositor operates on real artwork, so the editor's preview requires selecting a media item ID (the operator pastes one or picks from a recent item). A synthetic placeholder render is out of scope for the server compositing pipeline.
- **No new web dependencies** — the UI uses Svelte 5 runes, existing `core.js`, and native HTML inputs only; no color-picker, drag-drop, or JSON-schema library added.

**Verification:** `cargo check -p duskcue`, `cargo clippy -p duskcue --all-targets --all-features -- -A clippy::unnecessary-sort-by -D warnings` pass. `npm run check` + `npm run lint` pass in `clients/web`.
