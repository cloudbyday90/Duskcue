...
### Android TV Watch Next And Input Implementation — Phase 17 Tasks 7–9
...
The Android Keystore-encrypted mapping persists a hashed profile scope, Duskcue IDs, platform program ID, opaque content/render fingerprints, and opaque local-artwork metadata only. A profile/account/server/session change clears platform rows and app-private artwork before new profile data can publish. Changed rows alone are updated; absent/completed/revoked rows are deleted; failed cleanup retains only a numeric retry ID. The adapter responds to Android's disabled-program broadcast by deleting the row and suppressing an identical source fingerprint until the server feed has a legitimate content change. It refreshes from a fresh feed on active `tv_surface_changed`, resume, completion, exit, and a five-minute pause interval.

Task 8 resolves only a canonical API-relative poster URL for the canonical media UUID, requests the `w500` WebP variant through an in-memory bearer-header fetch, conditionally revalidates with ETag, and serves the resulting app-private file through an opaque UUID `content://` provider URI. It never publishes, persists, logs, or uses a bearer/signed/raw server URL as a provider URI. A changed ETag receives a new opaque URI and changes only that row; a `304` leaves the provider row untouched. Missing or unavailable remote art uses a deterministic title tile. Later app-local Android TV use has explicit backdrop `w1280`, thumbnail `w300`, and logo `original` choices through typed resolve artwork hints.
...
Phase 17 Task 13 additionally captures bounded physical-device context at support-bundle export: Android TV device family/model, Android release/API, display mode and advertised HDR types, advertised video decoder families, routed audio types/encodings, and coarse Ethernet/Wi-Fi state. This supports the NVIDIA SHIELD evidence plan in [NVIDIA_SHIELD_VALIDATION.md](../ci/NVIDIA_SHIELD_VALIDATION.md) without exposing serials, fingerprints, IP addresses, SSIDs, or content/account/profile data. It does not treat advertised capability as proof of real HDR, HDMI passthrough, AI upscaling, remote, standby, or launcher behavior.

Phase 17 Task 14 applies the same bounded diagnostic context to Sony BRAVIA while classifying Sony TV hardware as `sony_bravia`. [SONY_BRAVIA_VALIDATION.md](../ci/SONY_BRAVIA_VALIDATION.md) keeps Google TV and Android TV evidence separate: an ADB package/Leanback/deep-link preflight cannot prove model/region-specific Play visibility, home behavior, HDR/audio route, remote/voice behavior, standby/resume, or Watch Next placement. Those remain explicit protected physical observations for each target class.
