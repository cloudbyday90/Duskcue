...
Generation writes into a new artifact directory under the media-file directory while holding a transaction-scoped PostgreSQL advisory lock derived from `media_file_id`. It upserts the row and its `artifact_id` only after FFmpeg completes, so failed or cancelled work leaves the last complete set live. Rows created before artifact versioning retain a null `artifact_id` and resolve to the legacy `{media_file_id}/` layout until the next successful regeneration. Unreferenced artifact directories are cache data and are reconciled by the cache maintenance path.
...
- **`cache_dir` from `BootstrapConfig.data_dir`** — Until configured cache-root support is completed, handlers construct `state.bootstrap.data_dir.join("cache")` and pass it as `&Path` to service functions. Storyboards resolve through the row's optional `artifact_id`, using `{data_dir}/cache/storyboards/{media_file_id}/{artifact_id}/` for current artifacts and the legacy per-file directory for pre-versioning rows. Service signatures use `&Path` (not `&PathBuf`) per clippy `ptr_arg` convention.
