...
| 2 | **Image pipeline service** (`services/image_pipeline.rs` — WebP encode, resize, variant generation) | [IMAGE_FORMATS.md](IMAGE_FORMATS.md) | 3-5 days | **Phase 10** (storyboard WebP sprites use same service) | Artwork WebP variants; storyboard sprites; overlay composites (Phase 12) | Spec only |
| 3 | **Artwork delivery endpoint** (`GET /api/v1/items/{id}/artwork/{type}?size={size}`) | [IMAGE_FORMATS.md](IMAGE_FORMATS.md) | 2-3 days | **Phase 10** (shares `image_pipeline.rs`) | Web client poster rendering (currently gradient placeholders) | Spec only |
...
| Paraglide JS adoption — extract existing web client strings to `en.json`, configure Paraglide Vite plugin | #10 | 3-5 days |
