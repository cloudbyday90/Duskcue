...
- `clients/mobile/lib/stores/download_manager_store.dart` persists download inventory and settings metadata under a `(server_origin, user_id, profile_id, device_identifier)` scope so account, profile, server, or device switching cannot show another household profile's downloads. It loads on authenticated foreground, refreshes jobs after restart, merges `download_job_status` SSE events, and exposes queue, pause, resume, cancel, delete, delete-all, and retry actions.
...
- The Dart metadata writer strips bearer/session/access/refresh tokens, package keys, signed URLs, stream URLs, and transfer URLs before writing local package metadata. Download inventory/settings remain scoped by `(server_origin, user_id, profile_id, device_identifier)` and are cleared with the protected roots on local session cleanup.
...
Phase 16c Task 15 added observability and verification coverage:

- Download worker gauges: `download_queue_depth_jobs`, `download_active_jobs`, `download_bytes_prepared`, `download_storage_used_bytes`, and `download_retained_packages`.
- Download lifecycle counters/histograms: `download_package_bytes_prepared_total`, `download_package_files_per_package`, `download_transcode_duration_seconds`, `download_job_failures_total{reason,outcome}`, and `download_cleanup_total{kind}`. Failure labels are bounded categories such as `ffmpeg`, `checksum`, `storage`, `access`, `timeout`, `source_media`, and `other`.
- Package serving metrics: `download_package_file_transfer_total{role}` and `download_package_file_bytes_served_total{role,ranged}` are recorded after the handler reads the full or ranged response body, so interrupted-transfer diagnostics can compare requested files with actual bytes served.
- Reconnect sync metrics: `download_sync_submissions_total`, `download_sync_package_states_accepted_total`, `download_sync_playback_events_accepted_total`, `download_sync_conflicts_total`, and `download_sync_packages_rejected_total{reason}`. A sync conflict is counted when an accepted offline playback event does not advance resume state because the server already has watched state or a newer watch row from another device.
- Automated Rust coverage includes download planning helpers, package format/strategy selection, package-relative path traversal rejection, Range parsing/rejection, accepted offline event ID de-duplication/capping, sync conflict classification, and bounded failure-label normalization. Full database-backed lifecycle tests still depend on the PostgreSQL integration harness rather than unit-only SQL-free tests.
- Mobile model coverage includes scoped inventory keys, job status event merges, settings persistence for quality/network/storage cap/auto-delete watched, storage-cap clearing, manifest primary playback file selection, transfer checksum/Range hints, offline playable state persistence, and reconnect sync invalidation DTOs. Local Flutter/Dart SDK verification is now available for analyzer and unit-test coverage; physical-device transfer checks remain release-gate work.
- Manual verification cases for release validation: restart the server while a package is preparing and verify stale `preparing` jobs recover or fail cleanly; interrupt a transfer and verify HTTP Range resume plus checksum repair; revoke library access while a device is fully offline and verify the package remains local until reconnect and then becomes unavailable/deleted; create newer progress on a second device while the mobile client is offline and verify reconnect preserves the newer server resume state while accepting/deduplicating offline events.

## Mobile Contract

The mobile clients own these responsibilities:

- Keep download inventory scoped by server origin, server ID, user ID, profile ID, and device ID.
...
Phase 16c Tasks 0-15 are complete. The design/research outcome, database schema, downloads domain route/DTO/error shell, access/quota/policy foundations, deterministic planning endpoint, manifest response format, durable job creation/status/cancel, scheduled package worker, authenticated package serving, resumable transfer, foreground/push notifications, Flutter mobile download manager shell, protected Android/iOS local storage foundations, foreground package materialization, local MP4/HLS offline playback, idempotent reconnect sync, revocation/expiry invalidation, package renewal, server cleanup, user settings, admin policy settings, effective user/library overrides, inventory diagnostics, observability metrics, focused Rust/mobile model coverage, local Flutter analyzer/test verification, and manual release verification cases are in place. The native profile gate now scopes the Flutter inventory, protected package roots, and download settings by `profile_id` as well as server, user, and device; a profile switch immediately drops the in-memory manager state so packages from the prior scope are never rendered. Physical Android/iOS transfer validation and database-backed end-to-end lifecycle coverage remain release-gate work.
