...
Household profiles add a device-scoped post-auth preference contract: a client with a stable device ID may opt in to a remembered profile, while shared or incapable devices show normal profile selection. The device ID is non-secret and may be persisted only through platform-appropriate app storage; a client must never turn the preference into a profile credential or cache session/PIN material with it. Explicit sign-out and remote revocation clear the preference. Until parent unlock is implemented, a remembered Kids profile is content-filtered but is not a locked profile-exit mechanism.
