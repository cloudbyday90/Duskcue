...
| Check in the wrapper and align the Flutter Android target on JDK 17, AGP 8.11.1, Gradle 8.14, Kotlin 2.2.20, AndroidX, and Jetifier | Reproducible clean-checkout build; meets the dependency minimum and Flutter's current warning threshold. | Requires the wrapper download once per build environment and a focused build smoke test. | Selected. |
