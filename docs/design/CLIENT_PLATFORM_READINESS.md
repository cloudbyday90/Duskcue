...
| Fire TV | Amazon Fire TV getting-started, Appstore submission, Watch Activity, Content Personalization, Vega deep-link/content-personalization docs | Fire TV clients need Appstore metadata, physical-device testing, Android/Fire OS divergence notes, a customer-opt-in/partner-gated Watch Activity boundary, exact accepted catalog IDs, and separate Vega evaluation. The Phase 18 decision is [FIRE_TV.md](FIRE_TV.md). |
...
### Android Mobile Build Baseline (2026-07-18)

The checked-in Flutter Android target must be buildable from a clean workstation and CI runner without depending on a developer's generated Gradle files or an obsolete Android plugin. The Android SDK command-line tools install packages only through `sdkmanager` and require accepted licenses; Flutter then resolves the configured SDK and JDK for the project. Android's compatibility guidance lists JDK 17 for Android Gradle Plugin (AGP) 8.11 and Gradle 8.13 as its minimum wrapper version, while Flutter's current Android toolchain warning asks projects to move to AGP 8.11.1, Gradle 8.14, and Kotlin 2.2.20. The mobile dependency graph also now contains AndroidX Media3, so AndroidX and Jetifier must be explicitly enabled.

| Option | Advantages | Limitations | Decision |
|---|---|---|---|
| Keep a local/generated wrapper and old AGP 8.7.3 | No immediate project-file update. | A clean checkout is not reproducible; `androidx.core:core-ktx` rejects AGP 8.7.3; Flutter reports the toolchain as approaching unsupported. | Rejected. |
| Upgrade only AGP to the first dependency-compatible version | Resolves the immediate metadata error. | Leaves the project below Flutter's stated forward-compatibility baseline and still depends on an untracked wrapper. | Rejected. |
| Check in the wrapper and align the Flutter Android target on JDK 17, AGP 8.11.1, Gradle 8.14, Kotlin 2.2.20, AndroidX, Jetifier, and Flutter's standard 8 GB Gradle heap | Reproducible clean-checkout build; meets the dependency minimum and Flutter's current warning threshold. | Requires the wrapper download once per build environment and a focused build smoke test. | Selected. |

The local Android SDK is intentionally not repository state: each developer or CI runner configures its SDK path in ignored `android/local.properties` (or Flutter configuration), installs the required API/build-tools packages, and accepts licenses. The repository owns the Gradle wrapper and build-plugin baseline. The clean build also found a stale lockfile pairing in FlutterFire: `firebase_messaging` imports Android symbols supplied by its matching `firebase_core` release, so those two direct packages and their Firebase transitives must be upgraded together rather than independently. Flutter's upcoming built-in Kotlin DSL migration is deliberately deferred; the current project remains on the supported legacy plugin declaration while its build baseline is verified.

Sources rechecked on 2026-07-18: [Android `sdkmanager`](https://developer.android.com/tools/sdkmanager), [AGP 8.11 release notes](https://developer.android.com/build/releases/agp-8-11-0-release-notes), [AGP/Gradle compatibility](https://developer.android.com/build/releases/about-agp), [Flutter Android deployment](https://docs.flutter.dev/deployment/android), [Flutter's built-in Kotlin migration](https://docs.flutter.dev/release/breaking-changes/migrate-to-built-in-kotlin/for-app-developers), and the maintained [Firebase Messaging package releases](https://pub.dev/packages/firebase_messaging/changelog).
...
**Decision:** Phase 16d Task 3 adds `docs/api/fixtures/client/v1/manifest.json` plus focused JSON fixtures for server selection, auth/device-linking, preferences, libraries, media detail, search, household profile selection/switching/parent-unlock expiry, profile-scoped ambient channel handoff, collections, playback, subtitles/audio/storyboards/artwork, quality, downloads, notifications/settings, TV surface/deep-link behavior, and common denial cases. `scripts/verify-client-fixtures.mjs` is the drift gate for coverage, stable IDs, row ordering, UTC date-times, enum values, Problem Details shape, localized string ownership, and secret/path redaction.
...
The first Android/iOS implementation uses a native player as the ambient queue authority and a Flutter platform view only as its foreground display. Android declares the media-playback foreground-service type and uses Media3's stable 1.10.1 artifacts; iOS configures `AVAudioSession` with `.playback`/`.moviePlayback` while active and declares the `audio` background mode. Both native runtimes retain origin, bearer authorization, URLs, session IDs, and pending selection only in memory, disable system resumption across process loss, clear on profile/auth changes, and re-resolve after `PLAY_019`.

The Flutter Android/iOS client is the first native profile-contract consumer. Its router holds every new/restored authenticated session at a server-backed profile gate until `GET /api/v1/profiles` resolves; an enforced selection cannot leak normal routes, downloads, or cached artwork first. A successful switch clears decoded image cache and profile-scoped state, and local offline package/inventory/settings keys include `profile_id`. The client uses a transient obscured PIN field only to call parent unlock and immediately clears it. TV clients remain separately responsible for the same boundary when their platform targets are introduced.
