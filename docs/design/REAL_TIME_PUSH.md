...
| `download_job_status` | `domains/downloads/service.rs` and `workers/download_package_worker.rs` | [OFFLINE_DOWNLOADS.md](OFFLINE_DOWNLOADS.md) | User-scoped foreground status for offline download jobs; coalesced to queued/preparing/staged/ready/retry/failed/cancelled transitions |
