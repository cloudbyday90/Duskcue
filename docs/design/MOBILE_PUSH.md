...
- **Token-revoked-response invalidation deferred to Phase 16a** — Setting `invalidated_at`/`is_active = false` when FCM returns `UNREGISTERED` (404) or APNs returns `BadDeviceToken`/`Unregistered` requires the FCM/APNs HTTP client. Task 5 ships the schema + lifecycle API + staleness deactivation; provider-response invalidation lands with the push client in Phase 16a (matches the PHASE_13_SPLIT.md MVP boundary).
