...
The auth fixture includes a first-use multi-profile session, the `GET /api/v1/profiles` selection signal, and a switch that clears it. `scripts/verify-profile-selection-integration.mjs` binds the migration, session creation, transactional switch, API helpers, browser gate, cache cancellation, and same-origin synchronization into one regression check.
