...
- 1 expanded sub-config (Phase 13b Task 2): `NotificationConfig` — webhook dispatch config (`WebhookDispatchConfig { url, secret, format }`, `secret` encrypted at rest via `decrypt_notification_config()`/`encrypt_notification_config()`) + push dispatch config (`PushDispatchConfig { enabled, provider }`); defined in MOBILE_PUSH.md. `server_config.notifications = {}` deserializes to safe defaults (no webhook URL, generic format, push disabled). The webhook secret is decrypted in `load_runtime_config()` alongside other encrypted provider secrets.
...
- `server_config.notifications` includes push and webhook fields. As of Phase 13b Task 2, `NotificationConfig` is expanded from an empty placeholder to typed `WebhookDispatchConfig` (url, secret, format) + `PushDispatchConfig` (enabled, provider) sub-configs. The webhook `secret` is encrypted at rest via the existing `EncryptionKey`. The dispatch pipeline (`services/notification_dispatch.rs`) reads the live config to determine which channels to fan out to. Phase 13b is now complete — webhook dispatch is fully active (5 formats + HMAC signing + exponential-backoff retry); the admin UI "Activation requires Phase 13b" annotation has been removed from webhook fields. The push fields remain annotated "Mobile push client ships in Phase 16a" until the FCM/APNs/UnifiedPush HTTP clients are implemented; the `user_push_devices` table + registration API + preferences UI are all live, but actual push token delivery requires the Phase 16a mobile client.
