...
5. ~~Implement Android playback stack — Media3 ExoPlayer with HLS/direct/direct-stream/transcode handoff, `MediaSession`, remote/media-button handling, lifecycle pause/resume, progress heartbeat, seek/stop/completion reporting, cross-device resume refresh, audio/subtitle track selection, captions, segment skip controls, quality mode/status display, QoE telemetry, and playback-error reporting.~~ **DONE**
   ...
   **Task 5 implementation note:** Native `TvPlaybackService` is the app-internal Media3 `MediaSessionService` authority for one ExoPlayer, bearer-authenticated direct/HLS handoff, activity/task pause behavior, MediaSession/media buttons, 10-second backward and 30-second forward seeks, first-frame/periodic heartbeats, seek/stop/completion, fresh server resume handoff, final QoE, and safe player-error return. The detail surface loads profile-authorized media-file/segment metadata, offers audio/caption and quality choices, and presents an active-segment skip action. The server now persists every scanned audio stream, validates selected source indices as `VALID_001` field errors, maps selected audio to FFmpeg, burns selected captions when HLS conversion requires it, echoes restart-required selection state, and accepts fixture-aligned QoE failure metadata without a schema migration. `cargo check -p duskcue` and Android `:app:testDebugUnitTest :app:lintDebug :app:assembleDebug` pass. The next task owns launch/deep-link revalidation; duplicate-language direct-play selection and all real-device subtitle/remote validation remain release gates.
6. ~~Implement Android deep links and launch revalidation — `duskcue://play/{type}/{id}` and any Task 0-approved HTTPS/app-link path validate auth/access, refresh latest resume state, handle stale launcher tiles, resume after device-link auth, and enter playback directly without exposing raw IDs, tokens, or unavailable media details.~~ **DONE**
   ...
   **Task 6 implementation note:** `MainActivity` routes initial and subsequent custom-scheme intents through a strict in-memory parser. It accepts only `duskcue://play/{movie|episode}/{canonical UUID}` and rejects other authorities, types, path forms, queries, fragments, user-info, and ports before the API layer. Task 0 approved no deployment-specific HTTPS App Link, so no unsafe unverified host is registered. A valid target survives device linking/profile selection only in memory, resolves its canonical platform ID against the active profile, requires fresh `playable`/`access_revalidated` state, then creates a fresh playback session at the server’s resume position without an interstitial. Stale, revoked, malformed, or unavailable targets show the same bounded unavailable result; a 401 clears the session and resumes the non-secret target after renewed device linking. Raw URI data, IDs, tokens, URLs, and unavailable-item details are never persisted, rendered, or exposed in errors. Android unit tests cover canonical acceptance and capability-bearing/invalid rejection; `:app:testDebugUnitTest :app:lintDebug :app:assembleDebug` passes.
7. ~~Implement Android TV Watch Next adapter — fetch TV surface feed, map eligible continue/next-up/new-episode items to AndroidX `WatchNextProgram`, persist local `media_item_id` to platform `program_id` mappings, update only changed items, remove completed/revoked/stale rows, enforce one-item-per-series guidance, keep user/profile privacy boundaries, and react to `tv_surface_changed` refresh hints.~~ **DONE**

   **Task 7 implementation note:** Added `androidx.tvprovider:tvprovider:1.1.0`, an AndroidX `WatchNextProgram` publisher, and a pure reconciler with regression coverage. The active profile's fresh Android TV surface yields only playable traditional movies/episodes: an eligible continue item must exceed Android's start threshold and retain the end-credits fallback window; next/new rows require an episode `series_id`; source priority plus stable series IDs yields one episode per series. The server now returns nullable `series_id` on `TvSurfaceItemResponse`, and both fixture verifiers require it for episodes and reject it for movies. The encrypted Keystore/DataStore envelope stores only a hashed origin/user/profile scope, Duskcue identities, platform program ID, and an opaque fingerprint; it stores no stream URL, bearer token, raw scope, PIN, or copied resume state. Profile/account/server/logout cleanup stops playback and deletes every provider row before replacement data can publish. The publisher writes only changed rows, removes absent mappings, queues failed cleanup IDs, rejects stale cached surfaces, and treats Android's `ACTION_WATCH_NEXT_PROGRAM_BROWSABLE_DISABLED` as a delete plus same-fingerprint suppression until Duskcue emits an actual item change. Fresh publication runs after active `tv_surface_changed`, app resume, playback completion/exit, and a five-minute paused state. Artwork URI publication remains Task 8 so authenticated artwork delivery is introduced as one reviewed boundary.
...
8. ~~Add Android TV artwork and media metadata handling — choose poster/backdrop/thumbnail/logo dimensions per Android TV and Watch Next requirements, load authenticated/signed artwork without leaking credentials, respect cache headers/ETags, refresh after artwork changes, provide deterministic title-tile fallbacks, and populate TV Provider metadata fields with stable `platform_content_id` values.~~ **DONE**
   ...
   **Task 8 implementation note:** Watch Next poster publication now uses the authenticated Duskcue artwork endpoint without passing a bearer token, signed URL, or server URL to Android TV Provider. The client accepts only the canonical API-relative poster path for the candidate's canonical media UUID and requests the `w500` WebP variant with its in-memory bearer token, redirects disabled, a five-megabyte response cap, and `If-None-Match` when it has an ETag. The app-private cache writes a new opaque UUID file only for changed bytes; `304` retains the existing `content://com.duskcue.tv.watchnext-artwork/poster/{uuid}` URI, so only an actual image change changes the provider row. The exported provider accepts only a UUID poster path and serves app-private WebP files; it contains no raw Duskcue URL, credential, profile ID, path, or lookup endpoint. If remote artwork is absent or unreachable, a stable title/card tile is generated from the canonical content ID and title. The encrypted mapping envelope retains only a hashed scope, opaque cache key, source hash, and ETag. Profile/account/server/logout cleanup deletes the provider rows and the whole artwork cache. Poster (`w500`), backdrop (`w1280`), thumbnail (`w300`), and logo (`original`) variants are now explicit Android TV choices; `WatchNextProgram` receives the local poster URI while its existing title, description, series/season/episode, content ID, and internal provider ID remain stable. A source fingerprint separately protects disabled-row suppression from artwork-only refreshes, while a rendering fingerprint updates existing provider rows when the local poster URI changes. `TvResolveResponse` now retains its server artwork-hint object for later app-local artwork surfaces. Kotlin regression tests cover canonical-path rejection, changed-only local artwork publication, and disabled-row compatibility.
9. ~~Add accessibility, input, and Android TV quality checks — D-pad and gamepad focus traversal, Back behavior, overscan safety, 10-foot typography, visible focus/pressed/disabled states, TalkBack labels, captions/subtitle access, reduced-motion behavior, remote-control shortcuts, app startup/load error behavior, and Android TV app quality checklist coverage.~~ **DONE**

   **Task 9 implementation note:** `TvQualityPolicy` now defines conservative 58dp/28dp TV safe-area margins, 20sp minimum supporting text, logical in-app Back destinations, and explicit D-pad/gamepad/media/Menu/Captions player mappings. Compose controls have stable initial/restored focus, border-plus-surface focus/pressed/disabled states, TalkBack labels and live error announcements, no motion-dependent route/focus effect, and focused retry paths for bounded startup/load failures. Playback exposes visible play/pause, audio, and caption controls; changing a track during playback updates Media3 itself. `TvQualityPolicyTest` consumes the shared accessibility fixture pack and covers Back, remote mappings, layout/type policy, and Android TV review checklist cases. `node scripts/verify-accessibility-input.mjs` plus Android Gradle test/lint/build validate the automated boundary; physical TalkBack, overscan, reduced-motion, remote/gamepad, caption-preference, Watch Next launch-focus, and Google TV evidence remain explicit Task 11/12/13/14 gates.
10. ~~Add diagnostics and privacy-safe support evidence — client log schema, request/playback/session correlation IDs, playback diagnostics, Watch Next publication diagnostics, support bundle/export path where practical, redaction of tokens/signed URLs/private media paths, and user-visible troubleshooting that does not expose server internals.~~ **DONE**

   **Task 10 implementation note:** `TvDiagnostics` is an application-memory-only, 1,000-record/24-hour bounded ledger with the shared required client-log fields. API observations retain only host-only origin, route template, status, `x-request-id`, RFC 9457 `trace_id`, and a validated error code; raw URLs, request bodies, headers, bearer tokens, signed URLs, private paths, media IDs, titles, profile/account data, and filenames never enter a record or export. Profile/account/server cleanup clears the ledger. Playback diagnostics retain session ID, stream decision, and bounded error code; Watch Next records aggregate outcome counts plus opaque surface event IDs. Settings exports a redacted JSON bundle only through Android's user-selected document destination for manual sharing; there is no upload endpoint. Kotlin regressions cover redaction, retention, fixture checklist binding, and actual API request/trace correlation. `node scripts/verify-client-diagnostics.mjs` verifies the shared fixture pack; emulator/device document-picker handoff remains a Task 11/12 release gate.
11. ~~Add Android TV CI and conformance harness — Gradle lint/unit tests, fixture-backed API tests, TV/deep-link/playback/auth/accessibility/diagnostics verifier consumption, `node scripts/client-smoke-harness.mjs --plan`, `node scripts/verify-client-ci-smoke.mjs`, Android TV emulator smoke where feasible, and explicit manual/release-gate hardware checks where CI cannot run the device path.~~ **DONE**

   **Task 11 implementation note:** The shared client workflow now runs a path-scoped `android_tv_conformance` job on relevant PR/main changes. It pins Temurin 17 plus Android Platform/Build-Tools 36, consumes client/API/playback/auth/TV/deep-link/accessibility/diagnostics verifiers and the shared smoke-plan/CI verifiers, then executes Android `:app:testDebugUnitTest`, `:app:lintDebug`, and `:app:assembleDebug`. It uploads only short-retention debug APK/lint/unit-test troubleshooting artifacts; no release signing, publishing, SBOM, provenance, or release claim is made. An opt-in `android_tv_emulator_smoke` dispatch job boots an API 36 `android-tv` `tv_1080p` AVD and uses `scripts/android-tv-emulator-smoke.mjs` to verify `android.software.leanback`, APK installation, Leanback launcher entry, and valid-shape Duskcue deep-link delivery. `scripts/verify-android-tv-ci.mjs` keeps the workflow, fixture matrix, artifact paths, and emulator handoff wired. Real authenticated playback, Watch Next visibility, TalkBack, remote hardware, HDR/audio, standby/resume, document picker, Play, NVIDIA SHIELD, and Sony evidence remain explicit Tasks 12-14 release gates.
12. ~~Add Android TV release and Google Play readiness — Android TV form-factor track/readiness notes, target API policy, app bundle/APK artifact placeholders, package/versioning rules, Play signing placeholders, Data Safety/privacy disclosures, content rating, TV screenshots/banner/icon assets, reviewer test credentials/runbook, SBOM/provenance placeholders, rollback/update expectations, and Android TV quality checklist evidence.~~ **DONE**

   **Task 12 implementation note:** [ANDROID_TV_RELEASE_READINESS.md](docs/ci/ANDROID_TV_RELEASE_READINESS.md), `android-tv-google-tv-readiness.json`, and `verify-android-tv-release-readiness.mjs` now make the Android TV package/SDK policy, Gradle `duskcueVersionCode`/`duskcueVersionName` checks, candidate AAB/APK paths, dedicated Play TV-track decision, upload-key secret slots, Data Safety/content-rating/App Access ownership, reviewer runbook, SBOM/provenance placeholders, rollback policy, and manual quality/device gates executable documentation. The native runtime launcher banner is a real 320×180 non-transparent PNG, the Play banner is a real 1280×720 non-transparent PNG, and the Play icon is 512×512. The CI lane verifies this baseline but remains debug-only: Play Console registration, protected signing, final policy forms, reviewer credentials, real screenshots, Google TV visibility, 16 KB/ABI evidence, NVIDIA SHIELD, and Sony BRAVIA validation are deliberately still external Tasks 13-14 release gates.
13. Validate NVIDIA SHIELD TV as the high-capability Android TV reference device — test SHIELD TV and SHIELD TV Pro where available for Google Play visibility, Ethernet/Wi-Fi behavior, 4K HDR/HDR10/Dolby Vision mode behavior, Dolby Atmos/TrueHD/DTS passthrough and fallback, HLS/direct/direct-stream/transcode decisions, subtitles/captions, AI-upscaling/display-mode interactions, remote/gamepad controls, standby/resume, Watch Next behavior, and diagnostics capture.
   ...
   **Task 13 repository-side implementation note:** [NVIDIA_SHIELD_VALIDATION.md](docs/ci/NVIDIA_SHIELD_VALIDATION.md), `nvidia-shield-validation.json`, `nvidia-shield-validation.mjs`, and `verify-nvidia-shield-validation.mjs` now provide a non-secret SHIELD TV/Pro preflight, exact Ethernet/Wi-Fi/HDR/audio/AI/remote/standby/Watch Next/diagnostics evidence matrix, and protected evidence contract. Android TV support exports bounded display, HDR, decoder, routed audio, and coarse network capability context at the user-initiated diagnostics boundary. No SHIELD is attached to this workspace, so Task 13 remains open: no Play, HDR/Dolby Vision, TrueHD/DTS/Atmos, AI-upscaling, remote/gamepad, standby/resume, or Watch Next visibility observation is claimed until a physical SHIELD run records external evidence.
14. Validate Sony as Android TV / Google TV hardware — test Sony BRAVIA Google TV and Android TV devices for Google Play install visibility, Watch Next behavior, HLS playback, HDR, Dolby Vision/HDR10 behavior where supported, audio passthrough/downmix, subtitles, remote focus, standby/resume, voice/deep-link entry, and differences between Google TV and older Android TV BRAVIA models.
   ...
   **Task 14 repository-side implementation note:** [SONY_BRAVIA_VALIDATION.md](docs/ci/SONY_BRAVIA_VALIDATION.md), `sony-bravia-validation.json`, `sony-bravia-validation.mjs`, and `verify-sony-bravia-validation.mjs` now provide distinct Google TV/Android TV Sony BRAVIA preflight, exact Play/launcher/HLS/direct/HDR/audio/subtitle/remote/standby/voice/Watch Next/diagnostics evidence matrix, and protected evidence contract. Android TV support classifies Sony devices as `sony_bravia` and exports only bounded display, HDR, decoder, routed audio, and coarse network capability context at the user-initiated diagnostics boundary. No Sony BRAVIA is attached to this workspace, so Task 14 remains open: no Play, launcher, HDR/Dolby Vision, passthrough/downmix, remote/voice, standby/resume, or Watch Next observation is claimed until protected physical evidence covers both target UI generations.
