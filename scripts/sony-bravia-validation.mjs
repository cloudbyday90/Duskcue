...
const fixture = JSON.parse(fs.readFileSync(path.join(root, 'docs/api/fixtures/device-lab/v1/sony-bravia-validation.json'), 'utf8'));
...
assert(options.serial, 'A physical Sony BRAVIA ADB serial is required. Run with --plan to inspect the checklist.');
assert(['google_tv', 'android_tv'].includes(options.experience), '--experience must be google_tv or android_tv for a captured physical run.');
if (options.apk) {
...
const brand = getProp(adb, options.serial, 'ro.product.brand');
const model = getProp(adb, options.serial, 'ro.product.model');
assert(`${manufacturer} ${brand}`.toLowerCase().includes('sony'), `ADB target is not Sony hardware: ${safeLabel(`${manufacturer}_${brand}`)}`);
assert.notEqual(getProp(adb, options.serial, 'ro.kernel.qemu'), '1', 'ADB target is an emulator, not physical Sony BRAVIA hardware.');

const features = shell(adb, options.serial, ['pm', 'list', 'features']);
assert(features.includes('feature:android.software.leanback'), 'Sony target lacks android.software.leanback.');
const playStorePath = shell(adb, options.serial, ['pm', 'path', 'com.android.vending']);
assert(playStorePath.includes('package:'), 'Sony target does not expose the Google Play Store package. This does not test track visibility.');
...
const report = {
  ...
  device: {
    ...
    experience_declared: options.experience,
    ...
    google_play_store_package_present: true,
  },
  app: {
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (value === '--serial' || value === '--experience' || value === '--apk' || value === '--package') {
...
function printUsage() {
  console.log('Usage: node scripts/sony-bravia-validation.mjs --plan');
  console.log('       node scripts/sony-bravia-validation.mjs --serial <adb-serial> --experience <google_tv|android_tv> [--apk <path>] [--package com.duskcue.tv]');
}

function printPlan() {
  console.log(`Sony BRAVIA validation plan: ${fixture.status}`);
  ...
  console.log('No physical result is recorded by --plan. Use a connected BRAVIA with --serial and retain evidence outside the repository.');
}
